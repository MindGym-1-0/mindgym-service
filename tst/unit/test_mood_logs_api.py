...
# Import actual function implementations executed by FastAPI
...
def create_mock_chain(return_data: list) -> MagicMock:
    ...
    Prevents nested MagicMocks from triggering serialization issues inside route logging blocks.
    """
...
def test_create_mood_log_success(mock_supabase):
    ...
    mock_supabase.table.return_value = create_mock_chain([mock_record])

    # CORRECTION: Removed 'user_id' from the body payload to match updated MoodLogCreate contract
    payload = {"score": 8, "note": "Feeling productive today!"}
