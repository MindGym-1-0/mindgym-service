...
def generate_onboarding_script(
    ...
    from src.prompts.user_prompt import build_calibration_block

    # Derive emotional calibration from onboarding data so Maya's full rule set applies
    anxiety = baseline_anxiety or 5
    challenge_to_feeling = {
    ...
    current_feeling = challenge_to_feeling.get(emotional_challenge or "", "unsure")
    emotional_calibration = build_emotional_calibration(
        current_feeling=current_feeling,
        desired_feeling="grounded",
        anxiety_level_before=anxiety,
    )
    system = build_system_prompt(is_event_linked=False)
    ...
    prompt = f"{build_calibration_block(emotional_calibration)}\n\n{prompt}"
